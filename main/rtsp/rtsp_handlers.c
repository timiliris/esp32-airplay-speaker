...
static void parse_sdp(rtsp_conn_t *conn, const char *sdp, size_t len) {
    ...
    if (slash) {
      *slash = '\0';
      ...
      if (sscanf(slash + 1, "%d/%d", &sr, &ch) >= 1) {
        ...
        // Channel count comes from untrusted SDP; only accept a valid range,
        // otherwise keep the default of 2.
  ...
  if (fmtp) {
    ...
    if (matched >= 7) {
      ...
      // num_ch comes from untrusted SDP; only accept a valid range, otherwise
      // keep the default of 2 channels.
      if (num_ch >= 1 && num_ch <= AUDIO_MAX_CHANNELS) {
  ...
  if (rsaaeskey && aesiv_str) {
    ...
    // AirPlay v1 uses AES-128-CBC: the key and IV must be exactly 16 bytes.
    // Both come from untrusted SDP; reject any other size before using them as
    // crypto material (a short key/IV would leave uninitialised bytes in the
    // cipher state).
    if (rsa_decrypt_aes_key(key_b64, aes_key, sizeof(aes_key), &aes_key_len) ==
            0 &&
        aes_key_len == 16) {
      // Decode IV first so we can validate its length before committing.
      size_t iv_len = 0;
      ...
      if (iv_len == 16) {
        ...
        ESP_LOGW(TAG, "AirPlay v1: rejecting AES IV of %zu bytes (expected 16)",
                 iv_len);
      }
    } else {
      ESP_LOGW(TAG, "AirPlay v1: rejecting AES key of %zu bytes (expected 16)",
               aes_key_len);
    }
  }
#endif
...
static void handle_setup(int socket, rtsp_conn_t *conn,
  ...
  if (body && body_len > 0) {
    ...
    if (shk_len >= 16) {
      ...
      encryption_set = true;
    } else if (ekey_len > 16 && ekey_len <= 48 && conn->hap_session &&
               conn->hap_session->session_established) {
      // ekey is attacker-sized; the AEAD plaintext is (ekey_len - 16) bytes and
      // is written into a 32-byte buffer. Require ekey_len <= 48 (32-byte key +
      // 16-byte tag) so the plaintext can never overflow decrypted_key.
      uint8_t nonce[12] = {0};
 ...
 */
static void parse_dmap_metadata_depth(const uint8_t *data, size_t len,
                                      rtsp_metadata_t *meta, int depth) {
  // Bound recursion: attacker-controlled SET_PARAMETER data could nest
  // container tags arbitrarily deep and exhaust the stack.
  if (depth > 4) {
    return;
  }
  ...
  while (pos + 8 <= len) {
    ...
    if (strcmp(tag, "minm") == 0 && item_len > 0) {
      ...
      parse_dmap_metadata_depth(data + pos, item_len, meta, depth + 1);
    }
...
// Thin wrapper preserving the original signature for external callers.
static void parse_dmap_metadata(const uint8_t *data, size_t len,
                                rtsp_metadata_t *meta) {
  parse_dmap_metadata_depth(data, len, meta, 0);
}
