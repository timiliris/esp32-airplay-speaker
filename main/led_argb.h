...
/**
 * Optional addressable RGB LED strip (WS2812 / WS2812B / SK6812), audio-reactive.
 * Driven over the RMT peripheral. Replaces the MAX7219 matrix as the headline
 * "light effects" feature, but is independent of it.
 *
 ...
 * opt in.
 *
 * User-configurable: data GPIO, LED count, brightness, effect. Effects (argb_fx):
 *   0 = VU-metre, 1 = Spectre, 2 = Pulsation basses,
 *   3 = Arc-en-ciel (ambient), 4 = Veilleuse (ambient).
 */
 ...
 * led_audio_feed()/led_matrix_feed(). Returns immediately (cheap) when disabled.
 *
