...
static void process_timing_response(const uint8_t *packet, size_t len,
  ...
  // Copy into aligned locals before use: packet points at attacker-controlled
  // UDP bytes with no guaranteed 32-bit alignment, so cast-and-deref would be
  // unaligned-access UB. memcpy parses identically for valid packets.
  uint32_t raw[6];
  memcpy(raw, packet + 8, sizeof(raw));
  uint32_t origin_secs = ntohl(raw[0]);
  uint32_t origin_frac = ntohl(raw[1]);
  uint32_t receive_secs = ntohl(raw[2]);
  uint32_t receive_frac = ntohl(raw[3]);
  uint32_t transmit_secs = ntohl(raw[4]);
  uint32_t transmit_frac = ntohl(raw[5]);
